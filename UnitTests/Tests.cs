...
namespace Tests
{
    public class TestBase
    {
        ...
        {
            if (!bytesFrom.SequenceEqual(bytesTo)) Assert.Warn("Fixups are ordered differently!");
        }
        ...
        public virtual void NxToNx(string path)
        {
            ...
            var nxFromNxBytes = RoundtripBotwHKX(nxBytes, Path.GetExtension(path), HKXHeader.BotwNx());
        ...
        {
            var nxBytes = File.ReadAllBytes(path);
            var wiiuFromNxBytes = RoundtripBotwHKX(nxBytes, Path.GetExtension(path), HKXHeader.BotwWiiu());
        ...
        {
            var wiiuBytes = File.ReadAllBytes(path.Replace("/Switch/", "/WiiU/"));
            var wiiuFromWiiuBytes = RoundtripBotwHKX(wiiuBytes, Path.GetExtension(path), HKXHeader.BotwWiiu());
        ...
        {
            var wiiuBytes = File.ReadAllBytes(path.Replace("/Switch/", "/WiiU/"));
            var nxFromWiiuBytes = RoundtripBotwHKX(wiiuBytes, Path.GetExtension(path), HKXHeader.BotwNx());
